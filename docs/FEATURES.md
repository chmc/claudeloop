...
**Description:** Interactive nudge stops the current phase and collects user guidance via single-line input or `$EDITOR`. Guidance is injected at two positions in the retry prompt with strong directive framing. Forces standard retry strategy so guidance is not diluted by stripped/targeted tiers. Each nudge replaces the previous. Also available via `--nudge` CLI flag for scripted/pre-run use.
