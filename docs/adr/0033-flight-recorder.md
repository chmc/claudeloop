# 33. Flight Recorder — Self-contained HTML Execution Report
...
Implement a "flight recorder" that reconstructs execution data from existing `.claudeloop/` artifacts into a self-contained HTML file at `.claudeloop/replay.html`.
...
- **Shell extracts → JSON → JS renders:** `lib/recorder.sh` produces a JSON blob (spec in `docs/flight-recorder-spec.md`). The JSON is embedded in the HTML template at `assets/replay-template.html`.
