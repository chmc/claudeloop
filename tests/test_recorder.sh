...
@test "rec_extract_session: extracts permission denials when present" {
  local logfile="$TEST_DIR/denials.log"
  cat > "$logfile" << 'EOF'
[Session: model=claude-sonnet-4-20250514 cost=$0.0100 duration=10.0s turns=3 tokens=1000in/500out denials=2]
EOF
  result=$(rec_extract_session "$logfile")
  echo "$result" | grep -q '"permission_denials":2'
}
