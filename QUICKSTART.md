...
claudeloop --refactor                  # auto-refactor code after each phase (up to 5 attempts, preserves work between retries)
...
- `--refactor` adds up to 10 more API calls per phase (5 attempts × refactoring + verification each); up to 12 total with `--verify`
